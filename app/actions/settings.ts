...
export async function listOrganizationMembersWithEmail(orgSlug: string) {
  ...
  return memberships.map((m) => ({
    userId: m.userId,
    email: emails.get(m.userId) ?? "(email indisponível)",
  }));
}
...
export async function addMember(
  ...
  const user = await getCurrentUser();
  const organization = await getOrgForCurrentUser(orgSlug);
  if (!user || !organization) return { error: "Organização não encontrada." };

  const membership = await prisma.membership.findUnique({
    where: { userId_organizationId: { userId: user.id, organizationId: organization.id } },
  });
  if (!membership || !MANAGER_ROLES.includes(membership.role)) {
    return { error: "Só OWNER/ADMIN podem adicionar membros." };
  }
  ...
  const admin = createAdminClient();
  const { data, error } = await admin.auth.admin.listUsers({ perPage: 200 });
  if (error) return { error: "Erro ao consultar utilizadores." };
  const targetUser = data.users.find((u) => u.email?.toLowerCase() === email);
  if (!targetUser) {
    return { error: "Não existe nenhuma conta Nexa com esse email. A pessoa tem de se registar primeiro em /login." };
  }

  const existing = await prisma.membership.findUnique({
    where: { userId_organizationId: { userId: targetUser.id, organizationId: organization.id } },
  });
  if (existing) return { error: "Essa pessoa já é membro desta organização." };
...
async function assertCanManageMembers(organizationId: string) {
  ...
  if (!membership || !MANAGER_ROLES.includes(membership.role)) {
    throw new Error("Só OWNER/ADMIN podem gerir membros.");
  }
  return user;
}
...
export async function updateMemberRole(formData: FormData) {
  ...
  if (target.role === "OWNER" && roleRaw !== "OWNER") {
    ...
    if (ownerCount <= 1) throw new Error("A organização precisa de pelo menos um OWNER.");
  }
