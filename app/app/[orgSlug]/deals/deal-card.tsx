...
export function DealCard({ deal, orgSlug }: { deal: DealWithRelations; orgSlug: string }) {
  return (
    <div className="card elev-sm">
      <p className="card-title">{deal.title}</p>
      {deal.value !== null && (
        <p className="card-meta">{Number(deal.value).toLocaleString("pt-PT", { style: "currency", currency: "EUR" })}</p>
      )}
