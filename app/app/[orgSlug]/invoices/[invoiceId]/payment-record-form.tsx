...
export function PaymentRecordForm({ orgSlug, invoiceId }: { orgSlug: string; invoiceId: string }) {
  const t = useTranslations("Payments");
  const tMethods = useTranslations("PaymentMethods");
  const [state, action, isPending] = useActionState(recordPayment, { error: null });
    ...
    <form action={action} className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        <div className="field">
          ...
          <select id="method" name="method" className="input" defaultValue="BANK_TRANSFER">
